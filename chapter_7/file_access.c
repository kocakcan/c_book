 ...
 * operations is cat, which concatenates a set of named files into the standard
 * output. cat is used for printing files on the screen, and as a
 * general-purpose input collector for programs that do not have the capability
 ...
 *
 * The pointer, called the file pointer, points to a structure that contains
 ...
 * details, because the definitions obtained from <stdio.h> include a structure
 * declaration called FILE. The only declaration needed for a file pointer is
 * exemplified by
 *
 *  FILE *fp;
 ...
 * indicates how one intends to use the file. Allowable modes include read
 * ("r"), write ("w"), and append ("a"). Some systems distinguish between text
 * and binary files; for the letter, a "b" must be appended to the mode string.
 *
 ...
 * or EOF if an error occurs. Like getchar and putchar, getc and putc may be
 ...
 * declared in <stdio.h>. Normally stdin is connected to the keyboard and stdout
 * and stderr are connected to the screen, but stdin and stdout may be
 * redirected to files or pipes as described earlier.
 *
 ...
 *
 * For formatter input or output of files, the functions fscanf and fprintf may
 ...
 * With these preliminaries out of the way, we are now in position to write the
 * program cat to concatenate files. The design is one that has been found
 * convenient for many programs. If there are command-line arguments, they are
 * interpreted as filenames, and processed in order. If there are no arguments,
 ...
 *
 *  int fclose(FILE *fp)
 * is the inverse of fopen, it breaks the connection between the file pointer
 * and the external name that was established by fopen, freeing the file pointer
 * for another file. Since most operating systems have some limit on the number
 * of files that a program may have open simultaneously, it's a good idea to
 * free the file pointers when they are no longer needed, as we did in cat.
 * There is also another reason for fclose on an output file - it flushes the
 * buffer in which putc is collecting output. fclose is called automatically for
 * each open file when a program terminates normally. (You can close stdin and
 * stdout if they are not needed. They can also be reassigned by the library
 * function freopen.)
 */
#include <stdio.h>
