 ...
 * arguments. The format argument is described below; the other arguments, each
 * of which must be a pointer, indicate where the corresponding converted input
 * should be stored.
 *
 ...
 * many items were found. On the end of file, EOF is returned; note that this is
 * different from 0, which means that the next input character does not match
 * the first specification in the format string. The next call to scanf resumes
 ...
 * It scans the string according to the format in format and store the resulting
 * values through arg1, ag2, etc. These arguments must be pointers.
 *
 ...
 * - Ordinary characters (not %), which are expected to match the next non-whie
 ...
 * white space character or until field width, is specified, is exhausted. This
 * implies what scanf will read across boundaries to find its input, since
 ...
 * semantics of C. Below are the conversion characters:
 *
 * d    ; decimal integer; int *
 * i    ; integet; int *. The integer may be in octal (leading 0) or hexadecimal
 *      (leading 0x or 0X) o    ; octal integer (with or without leading zero);
 * int * u    ; unsigned decimal integer; unsigned int * x    ; hexadecimal
 * integer (with or without leading 0x or 0X); int * c    ; characters; char *.
 * The next input characters (default 1) are placed at the indicated spot. The
 * normal skip-ver white space is suppressed; to read the next non-white space
 * character, use %1s
 * s    ; character string (not quoted); char *, pointing to an array of
 * characters long enough for the string and a terminating '\0' that will be
 * added.
 * e, f, g; floating-point number with optional sign, optional decimal point and
 * optional exponent; float * %      ; literal %; no assignment is made.
 *
 * The conversion characters d, i, o, u, and x may be preceded by h to indicate
 * that a pointer to short rather than int appears in the argument list, or by l
 * (letter ell) to indicate that a pointer to long appears in the argument list.
 *
 * As a first example, the rudimentary calculator can be written with scanf to
 * do the input conversion:
 *
 * Suppose we want to read input lines that contains dates of the form
 *
 *  25 Dec 1988
 ...
 *
 * Literal characters can appear in the scanf format string; they must match the
 * same characters in the input. So we could read dates of the form mm/dd/yy
 ...
 * time, then pick it apart with scanf. For example, suppose we want to read
 * lines that contain a date in either of the forms above. Then we could write
 *
 ...
 * by scanf.
 *
 * A final warning: the arguments to scanf and sscanf must be pointers. By far
