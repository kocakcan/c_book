 ...
 * by fprintf goes to stderr, so it finds its way to the screen instead of
 ...
 * program execution when it is called. The argument of exit is available to
 * whatever process called this one, so the success or failure of the program
 ...
 * file, to flush out any buffered output.
 *
 * Within main, return expr is equivalent to exit(expr). exit has the advantage
 * that it can be called from other functions, and that calls to it can be found
 * with a pattern-searching program.
 *
 * The function ferror returns non-zero if an error occurred on the stream fp.
 *
 *  int ferror(FILE *fp)
 * Although output errors are rare, they do occur (for example, if a disk fills
 * up), so a production program should check this as well.
 *
 * The function feof(FILE *) is analogous to ferror; it returns non-zero if end
 * of file has occurred on the specified file.
 *
 *  int feof(FILE *fp)
