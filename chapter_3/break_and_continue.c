/*** - It is sometimes convenient to be able to exit from a loop other than by testing at the top or bottom. The break statement provides an early exit from for, while, and do, just as from switch. A break causes innermost enclosing loop or switch to be exited immediately.
- The following function, trim, removes trailing blanks, tabs and newlines from the end of a string, using a break to exit from a loop when the righmost non-blank, non-tab, non-newline is found.
*/
...
int main() {
	...
	if (trim(s) == -1)
		printf("This string either only contains white space characters or is empty.\n");
