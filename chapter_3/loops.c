 ...
 * when there are several nested loops.
 ...
 * L. Shell, is that in early stages, far-apart elements are compared, rather
 * than adjacent ones as in simpler interchange sorts.
 ...
 * it becomes zero. The middle loop steps along the elements. The innermost loop
 * compares each pair of elements that is seperated by gap and reverses any that
 * are out of order.
 * - Since gap is eventually reduced to one, all elements are eventually ordered
 * correctly.
 * - Notice how the generally of the for makes the outer loop fit in the same
 ...
 * for statement.
 * - A pair of expressions seperated by a comma is evaluated left to right, and
...
/***
 * - The commas that seperate function arguments, variables in declarations,
 ...
 * - A comma expression might also be appropriate for the exchange of elements
 * in reverse, where the exchange can be brought of a single operation: for (i =
 * 0, j = strlen(s) - 1; i < j; i++, j--) c = s[i], s[i] = s[j], s[j] = c;
 */
