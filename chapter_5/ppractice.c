...
Person *init_person(const char *name, int age, const char *occupation,
                    double balance) {
...
void printp(const Person *person, size_t width) {
  printf("Name: %-5s | Age: %d | Occupation: %-*s | Balance: $%.2f\n",
         person->name, person->age, (int)width, person->occupation,
         person->balance);
}
...
int main(void) {
  ...
  Person *sis =
      init_person("Dilan", 32, "Turkish Patent Attorney", random_doubles[3]);
  Person *bro = init_person("Medet", 32, "Metallurgical and Materials Engineer",
                            random_doubles[4]);
