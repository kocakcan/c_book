 ...
 *
 * In words, a dcl is a direct-dcl, perhaps preceeded by *'s. A direct-dcl is a name, or a paranthesized dcl, or a direct-dcl followed by parantheses, or a direct-dcl followed by brackets with an optional size.
 *
 * This grammar can be used to parse functions. For example, consider this declarator:
 *
 * 	(*pfa[])()
 * pfa will be identified as a name and thus as a direct-dcl. Then pfa[] is also a direct-dcl. Then *pfa[] is recognized as a dcl, so (*pfa[])() is a direct-dcl and thus a del.
 *
 * The heart of the dcl program is a pair of functions, dcl and dirdcl, that parse a declaration according to this grammar. Because the grammar is recursively defined, the functions call each other recursively as the recognize pieces of a declaration; the program is called a recursive-descent parser.
 */
...
/***
 * The function gettoken skips blanks and tabs, then finds the next token in the input; a "token" is a name, a pair of parantheses, a pair of brackets perhaps including a number, or any other single character.
 */
