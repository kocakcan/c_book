 ...
 * choose -x (for "except") to signal the inversion, and -n ("number") to
 * request line numbering, then the command
 *
 * 	find -x -npattern
...
/***
 * argc is decremented and argv is incremented before each optional argument. At
 * the end of the loop, if there are no errors, argc tells how many arguments
 * remain unprocessed and argv points to the first of these. Thus argc should be
 * 1 and *argv should point at the pattern. Notice that *++argv is a pointer to
 * an argument string, so (*++argv)[0] is its first character. (An alternate
 * valid form would be **++argv.) Because [] binds tighter than * and ++, the
 * parantheses are necessary; without them the expression would be taken as
 * *++(argv[0]). In fact, that is what we have used in the inner loop, the
 * expression *++argv[0] increments the pointer argv[0]!
 *
 * It is rare than one uses pointer expressions more complicated than these; in
