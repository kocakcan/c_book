 ...
 * filename. The second argument, flags, is an int that specifies how the file
 * is to be opened; the main values are
 *
 ...
 * creat os provided to create new files, or to re-write old ones.
 *
 *  int creat(char *name, int perms);
 *  fd = creat(name, perms);
 * returns a file descriptor if it was able to create file, and -1 if not. If
 * the file already exists, creat will truncate it to zero length, thereby
 * discarding its previous contents; it is an error to creat a file that already
 * exists.
 *
 ...
 * permissions. For example, 0775 specifies read, write and execute permission
 * for the owner, and read an execute permission for the group and everyone
 * else.
 *
 ...
 * permit the second argument to be a directory, and it invents permissions
 * instead of copying them.
 *
 * This program creates the output file with fixed permissions of 0666. With the
 * stat system call we can determine the mode of an existing file and thus give
 * the same mode to the copy.
 *
 ...
 * printf except that the variable argument list is replaced by a single
 * argument that has been initialized by calling the va_start macro. Similary,
 * vprintf and vsprintf match fprintf and sprintf.
 *
 *  #include <stdio.h>
 *  #include <stdarg.h>
 *
 ...
 *     exit(1);
 *  }
 * There is a limit (often about 200) on the number of files that a program may
 * open simultaneously. Accordingly, any program that intends to process many
 * files must be prepared to re-use file descriptors. The function close(int fd)
 * breaks the connection between a file descriptor and an open file, and frees
 * the file descriptor for use with some other file; it corresponds to fclose in
 * the standard library except that there is no buffer to flush. Termination of
 * a program via exit or return from the main program closes all open files.
 *
