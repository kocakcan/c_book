 ...
 * than bitwise logical operators. A bit-field, or field for short, is a set of
 * adjacent bits withing a single implementation-defined storage unit that we
 * will call a "word". For example, the symbol table #defines above could be
 * replaced by the definition of three fields:
 *
 *  struct {
 ...
 *  } flags;
 * This defines a variable table called flags that contain three 1-bit fields.
 * The number following the colon represents the field width in bits. The fields
 * are declared unsigned int to ensure that they are unsigned quantities.
 *
 * Individual fields are referenced in the same way as other structure members:
 * flags.is_keyword, flags.is_extern, etc. Fields behave like small integers,
 * and may participate in arithmetic expressions just like other integers. Thus
 * the previous examples may be written more naturally as
 *
 ...
 * internally-defined data structures, the question of which end comes first has
 * to be carefully considered when picking apart externally-defined data;
 * programs that depend on such thing are not portable. Fields may be declared
 * only as ints; for portability, specify signed or unsigned explicitly. They
 * are not arrays and they do not have addresses, so the & operator cannot be
 * applied on them.
 */
#include <stdio.h>
