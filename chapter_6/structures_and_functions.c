 ...
 *
 * 	struct point *pp;
 * says that pp is a pointer to a structure of type struct point. If pp points
 * to a point structure, *pp is the structure, and (*pp).x and (*pp).y are the
 * members. To use pp, we might write, for example,
 *
 * 	struct point origin, *pp;
 * 	pp = &origin;
 * 	printf("origin is (%d,%d)\n", (*pp).x, (*pp).y);
 * The parantheses are necessary in (*pp).x because the precedence of the
 * structure member operator . is higher than *. The expression *pp.x means
 ...
 * accessing len, and (p++)->len increments p afterward. (The last set of
 * parentheses are unnecessary)
 *
 * In the same way, *p->str fetches whatever str points to; *p->str++ increments
 * str after accessing whatever it points to (just like *s++); (*p->str)++
 * increments whatever str points to; and *p++->str increments p after accessing
 * whatever str points to.
 */
#include <stdio.h>
#define XMAX 1920
#define YMAX 1200
...
int main(void) {
  ...
  printf("pt4(%d, %d) %s in screen\n", pt4.x, pt4.y,
         (ptinrect(pt4, screen) == 1) ? "is" : "is not");
  printf("pt5(%d, %d) %s in screen\n", pt5.x, pt5.y,
         (ptinrect(pt5, screen) == 1) ? "is" : "is not");
  printf("canrect((%d, %d), (%d, %d))\n", canrect.pt1.x, canrect.pt1.y,
         canrect.pt2.x, canrect.pt2.y);
