 ...
 * single area of storage, without embedding any machine-dependent information
 * in the program. They are analogous to variant record in pascal.
 *
 ...
 * value of a particular constant must be stored in a variable of the proper
 * type, yet it is most convenient for table management if the value occupies
 * the same amount of storage and is stored in the same place regardless of its
 * type. This is the purpose of a union - a single variable that can
 * legitimately hold any one of several types. The syntax is based on
 * structures:
 *
 *  union u_tag {
 *    int ival;
 ...
 * the specific size is implementation-dependent. Any of these types may be
 * assigned to u and then used in expressions, so long as the usage is
 ...
 * current type stored in u, then on might see code such as
 *
 *  if (utype == INT)
 *    printf("%d\n", u.ival);
 *  if (utype == FLOAT)
 ...
 *
 *  symtab[i].u.ival
 * and the first characters of the string sval by either of
 *
 *  *symtab[i].u.sval
 *  symtab[i].u.sval[0]
