...
/***
 * Each call to getword finds a word, which is copied into the array named as its first argument.
 *
 ...
 *
 * getword fetches the next "word" from the input, where a word is either a string of letters and digits beginning with a letter, or a single non-white space character. The function value is the first character of the word, or EOF for end of file, or the character itself if is not alphabetic.
 */
