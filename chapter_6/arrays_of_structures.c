 ...
 *
 * Consider writing a program to count the occurrences of each C keyword. We
 * need an array of character strings to hold the names, and an array of
 ...
 * type, and sets aside storage for them. Each element of the array is a
 * structure. This could also be written
 *
 * 	struct key {
 * 		char *word;
 * 		int count;
 ...
 * or character strings, and when all are present. As usual, the number of
 * entries in the array keytab will be computed if the initializers are present
 * and the [] is left empty.
 *
 * The keyword counting program begins with the definition of keytab. The main
 * routine reads the input by repeatedly calling a function getword that fetches
 * one word at a time. Each word is looked up in keytab with a version of the
 * binary search function. The list of keywords must be sorted in increasing
 * order in the table.
 */
#include <ctype.h>
#include <stdio.h>
...
/***
 * Each call to getword finds a word, which is copied into the array named as
 * its first argument.
 *
 * The quantity NKEYS is the number of keywords in keytab. Although we could
 * count this by hand, it's a lot easier and safer to do it by machine,
 ...
 * structure. A type name can be the name of a basic type like int or double, or
 * a derived type like a structure or a pointer.
 *
 ...
 *
 * getword fetches the next "word" from the input, where a word is either a
 * string of letters and digits beginning with a letter, or a single non-white
 * space character. The function value is the first character of the word, or
 * EOF for end of file, or the character itself if is not alphabetic.
 */
...
/***
 * getword uses the getch and ungetch. When the collection of an alphanumeric
 * token stops, getword has gone one character too far. The call to ungetch
 * pushes that character back on the input for the next call. getword also uses
 * isspace to skip whitespace, isalpha to identify letters, and isalnum to
